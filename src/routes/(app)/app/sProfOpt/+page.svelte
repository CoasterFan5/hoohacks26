<script lang="ts">
	...
	const recommendations: Recommendation[] = [
		...
		},
		{
			...
			body:
				'Your 6-month average projected margin of ' +
				avgMargin +
				'% compares favorably to the regional benchmark of approximately 32–36% for operations of similar size. Maintaining current cost discipline is the primary lever.',
	...
	function barH(val: number) {
		return Math.max((val / projMax) * 100, 3);
	}
</script>
	...
	{#if activeTab === 'overview'}
		...
		<div class="kpi-grid">
			...
			<div class="kpi-card">
				...
				<div
					class="kpi-value"
					class:green={totalProjectedProfit >= 0}
					class:red={totalProjectedProfit < 0}
				>
			...
			<div class="overview-chart">
				{#each projections as p}
					<div class="ov-col">
						<div class="ov-bars">
							<div
								class="ov-bar income-bar"
								style="height:{barH(p.income)}%"
								title="Income: {fmt(p.income)}"
							></div>
							<div
								class="ov-bar expense-bar"
								style="height:{barH(p.expenses)}%"
								title="Expenses: {fmt(p.expenses)}"
							></div>
							<div
								class="ov-bar profit-bar"
								style="height:{barH(p.profit)}%"
								title="Profit: {fmt(p.profit)}"
							></div>
						</div>
		...
		<div class="callout-row">
			...
			<div class="callout callout-reco">
				...
				<button class="link-btn" onclick={() => (activeTab = 'recommendations')}
					...
					<div class="proj-col">
						...
						<div class="proj-bars">
							...
							<div
								...
								style="height:{barH(p.expenses)}%; animation-delay:{projections.indexOf(p) * 0.07 +
									0.03}s"
								title="Expenses: {fmt(p.expenses)}"
							></div>
						</div>
		...
		<div class="card">
			...
			<table class="data-table">
				...
				<tbody>
					{#each projections as p}
						<tr
							class:best-row={p.month === bestMonth.month}
							class:warn-row={p.month === worstMonth.month}
						>
							<td class="bold">{p.month}</td>
							<td class="mono green">+{fmt(p.income)}</td>
							<td class="mono red">−{fmt(p.expenses)}</td>
							<td class="mono" class:green={p.profit >= 0} class:red={p.profit < 0}
								>{fmt(p.profit)}</td
							>
	...
	{:else if activeTab === 'recommendations'}
		<div class="reco-summary">
			<span class="reco-count"
				>{recommendations.filter((r) => r.priority === 'High').length} High priority</span
			>
			<span class="reco-count-sep">·</span>
			<span class="reco-count"
				>{recommendations.filter((r) => r.priority === 'Medium').length} Medium priority</span
			>
			<span class="reco-count-sep">·</span>
			<span class="reco-count"
				>{recommendations.filter((r) => r.priority === 'Low').length} Low priority</span
			>
		</div>
		...
		<div class="reco-list">
			{#each recommendations as r}
				<div class="reco-card priority-border-{r.priority.toLowerCase()}">
					...
					<div
						class="reco-impact"
						class:impact-positive={r.impactDir === 'positive'}
						class:impact-neutral={r.impactDir === 'neutral'}
					>
	...
	{:else if activeTab === 'herd'}
		<div class="kpi-grid">
			...
			<div class="kpi-card">
				...
				<div class="kpi-sub">
					{herdDelta >= 0 ? '▲ Value trending up' : '▼ Value trending down'}
				</div>
			</div>
		</div>
				...
				<div class="herd-card">
					...
					<div class="herd-values">
						...
						<div class="herd-val-block">
							...
							<div
								class="herd-val-num"
								class:green={h.projectedValue >= h.currentValue}
								class:red={h.projectedValue < h.currentValue}
							>
					...
					<div class="value-bar-row">
						...
						<div class="value-bar-tracks">
							<div class="value-bar-track">
								<div
									class="value-bar-fill amber-bar"
						...
						<div class="value-bar-nums">
							...
							<span
								class="mono"
								class:green={h.projectedValue >= h.currentValue}
								class:red={h.projectedValue < h.currentValue}>{fmt(h.projectedValue)}</span
							>
						</div>
					</div>
				{/each}
...
<style>
	...
	.ov-bar {
		...
		animation: growBar 0.6s cubic-bezier(0.22, 0.68, 0, 1.2) forwards;
	}
	.ov-month {
		font-family: 'DM Mono', monospace;
		font-size: 9px;
	...
	.best-row td {
		background: rgba(90, 158, 90, 0.04) !important;
	}
	.warn-row td {
		background: rgba(212, 146, 42, 0.04) !important;
	}
	...
	.tag-monitor {
		background: rgba(127, 181, 200, 0.12);
		color: #7baec8;
		border: 1px solid rgba(127, 181, 200, 0.25);
	}
