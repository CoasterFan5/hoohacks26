<script lang="ts">
	...
	let herdValue  = $derived(herdGroups.reduce((s, g) => s + (g.count * g.avgWeightLbs * g.pricePerCwt) / 100, 0));
	...
	const recommendations = [
		{ icon: '💡', priority: 'High',   text: 'Feeder steer prices are at a 6-month high. Selling 10–15 head in the next 2 weeks could yield an estimated $2,800 above seasonal average.' },
		{ icon: '✂️', priority: 'Medium', text: 'Feed costs typically represent the largest single expense for operations your size. Lock in hay pricing before December to reduce estimated spend by 12–18%.' },
		{ icon: '📅', priority: 'Medium', text: 'Plan for a Q1 cash-flow dip — projections show reduced income Jan–Feb. Consider staging sales in Oct/Nov to build liquidity.' },
		{ icon: '📊', priority: 'Low',    text: 'Your projected Q2 margin of 35% is above regional average. Maintaining current herd size through spring should sustain this performance.' },
	];

	const incomeCategories  = ['Livestock Sale', 'Crop Sale', 'Government Payment', 'Custom Work', 'Insurance Payout', 'Other'];
	const expenseCategories = ['Feed', 'Fuel', 'Veterinary', 'Labor', 'Equipment', 'Land Lease', 'Seed / Chemicals', 'Insurance', 'Other'];
	...
	function submitIncome() {
		...
		incomeEntries = [...incomeEntries, {
			...
			category: incomeCategory, description: incomeDesc,
			amount: amt, date: incomeDate
		}];
		incomeSubmitted = true;
		incomeCategory = ''; incomeDesc = ''; incomeAmount = ''; incomeDate = today();
	...
	function submitExpense() {
		...
		expenseEntries = [...expenseEntries, {
			...
			category: expenseCategory, description: expenseDesc,
			amount: amt, date: expenseDate
		}];
		expenseSubmitted = true;
		expenseCategory = ''; expenseDesc = ''; expenseAmount = ''; expenseDate = today();
	...
	function addHerdGroup() {
		...
		herdGroups = [...herdGroups, { breed: newBreed, count, avgWeightLbs: weight, pricePerCwt: price }];
		newBreed = ''; newCount = ''; newWeight = ''; newPrice = '';
	}
...
<div class="page">
	...
	{#if activeTab === 'overview'}
		...
		<div class="kpi-grid">
			<div class="kpi-card">
				...
				<div class="kpi-value" class:green={totalIncome > 0} class:dim={totalIncome === 0}>{fmt(totalIncome)}</div>
				<div class="kpi-sub">{incomeEntries.length} {incomeEntries.length === 1 ? 'entry' : 'entries'}</div>
			</div>
			<div class="kpi-card">
				<div class="kpi-label">Total Expenses</div>
				<div class="kpi-value" class:red={totalExpenses > 0} class:dim={totalExpenses === 0}>{fmt(totalExpenses)}</div>
				<div class="kpi-sub">{expenseEntries.length} {expenseEntries.length === 1 ? 'entry' : 'entries'}</div>
			</div>
			<div class="kpi-card">
				<div class="kpi-label">Net Profit</div>
				<div class="kpi-value" class:green={netProfit > 0} class:red={netProfit < 0} class:dim={netProfit === 0}>{fmt(netProfit)}</div>
		...
		<div class="action-grid">
			<div class="action-card" onclick={() => activeTab = 'income'} role="button" tabindex="0">
				...
				<div class="action-desc">Record a livestock sale, crop sale, government payment, or other income.</div>
				<div class="action-arrow">→</div>
			</div>
			<div class="action-card" onclick={() => activeTab = 'expenses'} role="button" tabindex="0">
				<div class="action-icon">🧾</div>
				<div class="action-label">Log Expense</div>
				<div class="action-desc">Record feed, fuel, vet costs, labor, equipment, or other expenses.</div>
				<div class="action-arrow">→</div>
			</div>
			<div class="action-card" onclick={() => activeTab = 'herd'} role="button" tabindex="0">
				<div class="action-icon">🐄</div>
				<div class="action-label">Update Herd</div>
				<div class="action-desc">Update head count, weights, and market prices to refresh your valuation.</div>
				<div class="action-arrow">→</div>
			</div>
			<div class="action-card" onclick={() => activeTab = 'projections'} role="button" tabindex="0">
				<div class="action-icon">📈</div>
				<div class="action-label">View Projections</div>
				<div class="action-desc">See 6-month profitability forecasts and profit maximization recommendations.</div>
				<div class="action-arrow">→</div>
			</div>
		</div>
		...
		<!-- Placeholder notice -->
		<div class="notice">
			⚠ All figures shown are placeholder values. Log income and expenses using the tabs above to see your real numbers.
		</div>
	...
	{:else if activeTab === 'income'}
		...
		<div class="form-page">
			<div class="form-card">
				...
				<button class="btn-primary wide" onclick={submitIncome}>
					Save Income Entry
				</button>
			</div>
	...
	{:else if activeTab === 'herd'}
		...
		<div class="card">
			...
			<div class="herd-note">
				Valuation formula: Head × Avg Weight (lbs) × Price/CWT ÷ 100. Edit any field and the total updates instantly.
			</div>
					...
					<div class="herd-row">
						...
						<div class="herd-fields">
							...
							<div class="herd-field">
								...
								<input class="inline-input" type="number" bind:value={herdGroups[i].avgWeightLbs} min="0" />
							</div>
							<div class="herd-field">
								<div class="herd-field-label">$/CWT</div>
								<input class="inline-input" type="number" bind:value={herdGroups[i].pricePerCwt} min="0" />
							</div>
							<div class="herd-field">
			...
			<div class="herd-add">
				...
				<div class="herd-add-fields">
					<div class="form-group">
						...
						<input class="form-input" id="nb" type="text" placeholder="e.g. Hereford Steers" bind:value={newBreed} />
					</div>
					<div class="form-group">
						<label class="form-label" for="nc">Head Count</label>
						<input class="form-input" id="nc" type="number" placeholder="0" min="0" bind:value={newCount} />
					</div>
					<div class="form-group">
						<label class="form-label" for="nw">Avg Weight (lbs)</label>
						<input class="form-input" id="nw" type="number" placeholder="800" min="0" bind:value={newWeight} />
					</div>
					<div class="form-group">
						<label class="form-label" for="np">Price / CWT ($)</label>
						<input class="form-input" id="np" type="number" placeholder="180" min="0" bind:value={newPrice} />
					</div>
				</div>
	...
	{:else if activeTab === 'projections'}
		...
		<div class="card">
			...
			<div class="proj-disclaimer">
				⚠ Placeholder data — connect your backend to generate real forecasts based on your logged income and expenses.
			</div>
		</div>
...
<style>
	...
	.action-label {
		...
		font-weight: 600; color: var(--wheat);
	}
	.action-desc { font-size: 12px; color: var(--muted); line-height: 1.6; flex: 1; }
	.action-arrow { color: var(--rust); font-size: 16px; transition: transform .18s; }
	...
	.form-input option { background: var(--bark); }
	...
	.btn-primary {
		...
		transition: background .18s, transform .15s; width: fit-content;
	}
	...
	.btn-secondary-submit {
		...
		border: 1px solid rgba(212,146,42,.35); border-radius: 5px;
		padding: 11px 22px; font-family: 'DM Sans', sans-serif;
		font-size: 14px; font-weight: 500; cursor: pointer;
		transition: border-color .18s, background .18s; width: fit-content;
	}
	.btn-secondary-submit:hover { border-color: var(--amber); background: rgba(212,146,42,.06); }
	.btn-secondary-submit.wide  { width: 100%; }
	...
	.summary-total {
		...
		padding-top: 12px; border-top: 1px solid rgba(212,146,42,.15);
	...
	.herd-list { display: flex; flex-direction: column; gap: 12px; }
	.herd-row {
	...
	.herd-fields {
		display: flex; gap: 14px; flex: 1; flex-wrap: wrap; align-items: flex-end;
	}
	.herd-field { display: flex; flex-direction: column; gap: 5px; }
	.herd-field-label {
	...
	.inline-input {
		...
		font-family: 'DM Mono', monospace; width: 88px;
		transition: border-color .18s;
	}
	.inline-input:focus { outline: none; border-color: var(--amber); }

	.herd-add {
		border-top: 1px solid rgba(212,146,42,.12);
		padding-top: 20px;
		display: flex; flex-direction: column; gap: 14px;
	}
	.herd-add-fields {
	...
	.income-dot  { background: var(--sage); }
	.expense-dot { background: var(--rust); }
	.proj-disclaimer {
		font-size: 11px; color: var(--muted);
		background: rgba(212,146,42,.04);
		border: 1px solid rgba(212,146,42,.1);
		border-radius: 5px; padding: 10px 14px; line-height: 1.6;
	}
	...
	.ledger td {
		padding: 11px 10px; color: var(--cream);
		border-bottom: 1px solid rgba(255,255,255,.04);
	}
	.ledger tr:last-child td { border-bottom: none; }
	.ledger tr:hover td { background: rgba(212,146,42,.02); }
	...
	.reco-icon { font-size: 16px; flex-shrink: 0; margin-top: 2px; }
	.reco-body { display: flex; flex-direction: column; gap: 6px; }
	.reco-priority {
		font-family: 'DM Mono', monospace; font-size: 9px;
		letter-spacing: .1em; text-transform: uppercase;
	...
	.reco-text { font-size: 13px; color: var(--muted); line-height: 1.65; }
	...
	.del-btn {
		...
		color: rgba(196,64,64,.35); font-size: 13px;
		padding: 4px 8px; border-radius: 4px; flex-shrink: 0;
		transition: color .18s, background .18s;
	}
	.del-btn:hover { color: var(--red); background: rgba(196,64,64,.1); }

	/* ── RESPONSIVE ── */
	@media (max-width: 640px) {
		.form-row         { grid-template-columns: 1fr; }
		.herd-add-fields  { grid-template-columns: 1fr 1fr; }
