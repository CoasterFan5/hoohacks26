...
export const getExpensesData = query(async () => {
	const u = await handleAuth();

	const financeEntries = await db
		.select()
		.from(financeEntriesTable)
		.where(and(eq(financeEntriesTable.userId, u.id), eq(financeEntriesTable.type, 'expense')))
...
export const saveExpense = command(
	z.object({
